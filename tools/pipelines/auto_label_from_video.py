...
def copy_and_clean_labels(src_dir, dst_dir):
...
def write_classes_file(labels_dir, classes_text):
    if not classes_text:
        return
...
def parse_args():
    parser = argparse.ArgumentParser(description="抽帧 + 自动打标总流程")
    ...
    parser.add_argument("--labels-dir", default="data/labels_auto", help="自动标注输出目录")
    parser.add_argument("--project", default="runs/detect", help="推理输出根目录")
    parser.add_argument("--name", default="auto", help="推理输出名称")
    parser.add_argument("--conf", type=float, default=0.25, help="置信度阈值")
    parser.add_argument("--iou", type=float, default=0.45, help="NMS IOU 阈值")
    parser.add_argument("--classes", default="", help="classes.txt 的类别列表，逗号分隔")
...
def main():
    ...
    detect_script = root_dir / "yolov5" / "detect.py"
    ...
    if not detect_script.exists():
        raise FileNotFoundError(f"找不到 detect.py: {detect_script}")
    ...
    copy_and_clean_labels(labels_src, labels_dir)
    write_classes_file(labels_dir, args.classes)
